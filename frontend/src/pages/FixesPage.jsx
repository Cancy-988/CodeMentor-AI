      ...
      <div className="relative mx-auto max-w-[1440px] px-4 py-8 sm:px-6 lg:px-10">
        <section className="rounded-[32px] border border-[var(--color-border)] bg-[var(--color-bg-secondary)]/88 p-5 shadow-[0_30px_100px_rgba(0,0,0,0.18)] backdrop-blur-xl sm:p-6">
          ...
          <ul className="mt-6 space-y-4">
            {fixes.map((fix) => (
              <li key={fix.id} className="rounded-[24px] border border-[var(--color-border-light)] bg-[var(--color-bg-tertiary)]/85 p-5 shadow-[0_18px_60px_rgba(0,0,0,0.2)]">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                  <div className="space-y-2">
                    <div className="flex flex-wrap items-center gap-2">
                      <h3 className="text-lg font-semibold text-[var(--color-text-primary)]">Fix #{fix.id}</h3>
                    ...
                    <p className="text-sm leading-6 text-[var(--color-text-secondary)]">{fix.explanation || 'Multi-agent fix suggestion saved from the workspace.'}</p>
                    <p className="text-sm leading-6 text-[var(--color-text-tertiary)]">Original code preview: {fix.original_code?.slice(0, 120)}</p>
                  </div>

                  <div className="text-sm text-[var(--color-text-tertiary)]">{fix.created_at ? new Date(fix.created_at).toLocaleString() : ''}</div>
                </div>
              </li>
            ))}
          </ul>
